...
switch ($method) {
  case 'GET':
		$sql = "select repciDcs.repciId, repciDcTuplas.idDC, repciDcTuplas.value1,repciDcTuplas.value2 from repciDcTuplas,repciDcs where  repciDcs.DCType='Recordatorio' and repciDCTuplas.idDC = repciDcs.id ";
		...
		if ($start!=="" || $end!==""){
			...
			if ($start!==""){
				$subsql = " select repciDcs.id from repciDcs,repciDCTuplas where repciDcs.id=repciDcTuplas.idDC and repciDcTuplas.value1='FechaRecordatorio' and repciDcTuplas.value2>='$start'";
			}
			if ($end!==""){
				if ($start!==""){
					$subsql=$subsql." INTERSECT ";
				}
				$subsql = $subsql." select repciDcs.id from repciDcs,repciDCTuplas where repciDcs.id=repciDcTuplas.idDC and repciDcTuplas.value1='FechaRecordatorio' and repciDcTuplas.value2<='$end'";
			}
		}
		$sql=$sql.$subsql;
		if ($start!=="" || $end!==""){
...
if (!sqlsrv_has_rows($result)){
		...
		while( sqlsrv_fetch( $result )) {
		   ...
		   if ($desc=='ObsRecordatorio'){
			   $desc='title';			   
		   }
		   if ($desc=='FechaRecordatorio'){
			   $desc='start';			   
		   }

...
switch ($method) {
  case 'GET':
		...
		if ($start!=="" || $end!==""){
			...
			if ($start!==""){
				$subsql = " select repciDcs.id from repciDcs,repciDCTuplas,repcis where repciDcs.id=repciDcTuplas.idDC  and repciDcs.repciid=repcis.repciid and repcis.status=1  and repciDcs.OriginalDC is Null and (repciDcTuplas.value1='FechaCompromiso') and repciDcTuplas.value2>='$start'";
			}
			if ($end!==""){
				if ($start!==""){
					$subsql=$subsql." INTERSECT ";
				}
				$subsql = $subsql." select repciDcs.id from repciDcs,repciDCTuplas,repcis where repciDcs.id=repciDcTuplas.idDC  and repciDcs.repciid=repcis.repciid and repcis.status=1 and repciDcs.OriginalDC is Null and (repciDcTuplas.value1='FechaCompromiso') and repciDcTuplas.value2<='$end'";
			}
		}
		$sql=$sql.$subsql;
		if ($start!=="" || $end!==""){

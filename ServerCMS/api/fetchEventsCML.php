...
switch ($method) {
  case 'GET':
		$sql = "select repciDcs.repciId, repciDcTuplas.idDC, repciDcTuplas.value1,repciDcTuplas.value2,repciDcs.comment from repciDcTuplas,repciDcs where  repciDcs.DCType='".$langtext["Cita"]."' and repciDCTuplas.idDC = repciDcs.id ";
		
		$sql = $sql." and repciDcs.status=0 ";//and repciDcs.repciid = repcis.repciid and repciid.status=1 ";
		$sqlGrupo = "";
		if ($userid!==''){
		...
		if ($start!=="" || $end!==""){
			...
			if ($start!==""){
				$subsql = " select repciDcs.id from repciDcs,repciDCTuplas where repciDcs.id=repciDcTuplas.idDC and repciDcTuplas.value1='".$langtext["FechaCita"]."' and repciDcTuplas.value2>='$start'";
			}
			if ($end!==""){
				if ($start!==""){
					$subsql=$subsql." INTERSECT ";
				}
				$subsql = $subsql." select repciDcs.id from repciDcs,repciDCTuplas where repciDcs.id=repciDcTuplas.idDC and repciDcTuplas.value1='".$langtext["FechaCita"]."' and repciDcTuplas.value2<='$end'";
			}
		}
		$sql=$sql.$subsql;
		if ($start!=="" || $end!==""){

...
switch ($method) {
  case 'GET':
		...
		if ($start!=="" || $end!==""){
			...
			if ($start!==""){
				$subsql = " select repciDcs.id from repciDcs,repciDCTuplas,repcis where repciDcs.id=repciDcTuplas.idDC  and repciDcs.repciid=repcis.repciid and repcis.status=1  and repciDcs.OriginalDC is Null and ( repciDcTuplas.value1='".$langtext["FechaAviso"]."' or repciDcTuplas.value1='".$langtext["FechaCumple"]."' ) and repciDcTuplas.value2>='$start'";
			}
			if ($end!==""){
				if ($start!==""){
					$subsql=$subsql." INTERSECT ";
				}
				$subsql = $subsql." select repciDcs.id from repciDcs,repciDCTuplas,repcis where repciDcs.id=repciDcTuplas.idDC  and repciDcs.repciid=repcis.repciid and repcis.status=1 and repciDcs.OriginalDC is Null and ( repciDcTuplas.value1='".$langtext["FechaAviso"]."' or repciDcTuplas.value1='".$langtext["FechaCumple"]."' ) and repciDcTuplas.value2<='$end'";
			}
		}
		$sql=$sql.$subsql;
		if ($start!=="" || $end!==""){
		...
		while( sqlsrv_fetch( $result )) {
		   ...
		   if ($idDC !== $dummDC && $idDC!==''){
				...
				if (strpos($entry,'".$langtext["FechaVencimiento"].":')){
					...
					if (strpos($entry,'".$langtext["FechaCumple"].":')){
						$entry=$entry.',';
						$entry=$entry.str_replace('".$langtext["FechaCumple"].":', '"start":',$orgentry);
						if (strpos($entry,'".$langtext["FechaCompromiso"].":')){
							$entry=$entry.',';
							$entry=$entry.str_replace('".$langtext["FechaCompromiso"].":', '"start":',$orgentry);
						}
					...
					if (strpos($entry,'"$langtext["FechaAviso"]":')){
						...
						if (strpos($entry,'".$langtext["FechaCompromiso"].":'))
							...
							if (strpos($entry,'".$langtext["FechaCumple"].":'))
								$entry=str_replace('".$langtext["FechaCumple"].":', '"start":',$entry);
					}
				}				
			}
			if (!$first_row){
